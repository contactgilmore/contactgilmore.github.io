...
- For local browser smoke tests, first reuse an existing shared/local Playwright installation and its matching browser binaries when that environment is known. The owner-side shared Playwright environment is `~/Dev/dependencies/playwright`; prefer that existing environment before attempting a new Playwright or Chromium install. If the shared environment cannot satisfy the current project version, explain the mismatch and then use the least-destructive corrective step.
